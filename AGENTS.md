...
- Exercise details are optional read-only metadata from `exercises`, `session_exercises`, and `exercise_sets`; databases without those tables must still render sessions normally. A Daily Form with Exercise Details must have exactly one session whose optional type is `exercise`, and every exercise row in that form attaches to that session.
...
- The plugin natively parses, validates, previews, and imports official Data Schema v1 Daily Notes, current/future planning projections, Meals, admin events, milestones, and weekly plans. It also materializes weekly sessions into guarded Daily Note edits. Python is not a plugin runtime dependency.
