...
import { labeledValue as findLabeledValue, splitPipeFields } from '../forms/fields.ts';
import {
...
/** @deprecated Use BudgetFormInput. */
export type NativeBudgetFormInput = BudgetFormInput;
...
function formBody(sourceText: string): string {
  return requireEhForm(
    sourceText,
    'budget',
    'This note does not contain an EH Budget Form heading.',
    'The EH Budget Form has no matching #### END marker.',
  ).body;
}

function labeledValue(text: string, label: string): string {
  const value = findLabeledValue(text, label);
...
function sections(body: string): Map<string, string> {
  ...
  for (const [name, section] of formSections(body)) result.set(name, section.content);
  return result;
}

function entries(section: string | undefined, label: string): string[] {
  if (!section) return [];
  if (!/^ENTRIES:[ \t]*$/mi.test(section)) throw new Error(`${label} has no ENTRIES marker.`);
  return entryLines(section);
}
...
export function parseBudgetForm(db: Database, input: BudgetFormInput): ParsedBudgetForm {
...
export function inspectBudgetForm(db: Database, input: BudgetFormInput): BudgetImportResult {
...
export function writeBudgetForm(db: Database, input: BudgetFormInput): BudgetImportResult {
  const parsed = parseBudgetForm(db, input);
  assertBudgetDoesNotOverlap(db, parsed.periodStart, parsed.periodEnd);
