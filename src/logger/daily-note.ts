...
/** @deprecated Use DailyNoteInput. */
export type NativeDailyNoteInput = DailyNoteInput;
/** @deprecated Use DailyInspection. */
export type NativeDailyInspection = DailyInspection;
/** @deprecated Use DailyImportResult. */
export type NativeDailyImportResult = DailyImportResult;
...
function dailyFormBody(sourceText: string, expectedDate: string): string {
  ...
  const body = requireEhForm(
    text,
    'daily',
    'The note does not contain an EH Daily Form heading.',
    'The note does not contain a #### END marker.',
  ).body;
...
function sectionsFromForm(sourceText: string, expectedDate: string): Map<string, string> {
  ...
  for (const [name, section] of formSections(body)) result.set(name, section.content.trim());
  return result;
}

function entries(section: string | undefined): string[] {
  return section ? entryLines(section) : [];
}
...
function prepareDaily(db: Database, input: DailyNoteInput): PreparedDailyNote {
  ...
  if (errors.length === 0) applyAdminEvents(db, parsed.adminEvents, input.noteDate, errors);
  if (errors.length === 0) {
