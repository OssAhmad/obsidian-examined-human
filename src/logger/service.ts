...
export interface MealImportRequest extends Omit<MealImportInput, 'sourceChecksum' | 'pluginVersion'> {
  databasePath: string;
  sourceText: string;
}

export interface MealWriteResult extends MealImportResult, BackupMutationMetadata {
  databasePath: string;
  sourceChecksum: string;
}

export interface MealInspectionRequest {
  databasePath: string;
  sourceText: string;
...
export interface BudgetRequest extends Omit<BudgetFormInput, 'sourceChecksum'> {
  databasePath: string;
}

export interface BudgetWriteResult extends BudgetImportResult, BackupMutationMetadata {
  databasePath: string;
  sourceChecksum: string;
}

export interface DailyRequest extends Omit<DailyNoteInput, 'sourceChecksum' | 'pluginVersion'> {
  databasePath: string;
}

export interface DailyWriteResult extends DailyImportResult, BackupMutationMetadata {
  databasePath: string;
  sourceChecksum: string;
}

export type PlanningNoteRequest = Omit<PlanningNote, 'sourceChecksum'>;

export interface PlanningRequest {
  databasePath: string;
  cutoffDate: string;
  notes: PlanningNoteRequest[];
}

export interface PlanningDateRequest {
  databasePath: string;
  noteDate: string;
  note: PlanningNoteRequest | null;
}

export interface WeeklyRequest extends Omit<WeeklyNoteInput, 'sourceChecksum'> {
  databasePath: string;
}

export interface WeeklyWriteResult extends WeeklyImportResult, BackupMutationMetadata {
  databasePath: string;
  sourceChecksum: string;
}

/** @deprecated Use MealImportRequest. */
export type NativeMealImportRequest = MealImportRequest;
/** @deprecated Use MealWriteResult. */
export type NativeMealImportResult = MealWriteResult;
/** @deprecated Use MealInspectionRequest. */
export type NativeMealInspectionRequest = MealInspectionRequest;
/** @deprecated Use BudgetRequest. */
export type NativeBudgetRequest = BudgetRequest;
/** @deprecated Use BudgetWriteResult. */
export type NativeBudgetWriteResult = BudgetWriteResult;
/** @deprecated Use DailyRequest. */
export type NativeDailyRequest = DailyRequest;
/** @deprecated Use DailyWriteResult. */
export type NativeDailyWriteResult = DailyWriteResult;
/** @deprecated Use PlanningNoteRequest. */
export type NativePlanningNoteRequest = PlanningNoteRequest;
/** @deprecated Use PlanningRequest. */
export type NativePlanningRequest = PlanningRequest;
/** @deprecated Use PlanningDateRequest. */
export type NativePlanningDateRequest = PlanningDateRequest;
/** @deprecated Use WeeklyRequest. */
export type NativeWeeklyRequest = WeeklyRequest;
/** @deprecated Use WeeklyWriteResult. */
export type NativeWeeklyWriteResult = WeeklyWriteResult;
...
export class LoggerService {
  ...
  async inspectBudget(request: BudgetRequest): Promise<BudgetImportResult> {
    const input = await this.budgetInput(request);
    return this.inspectDatabase(request.databasePath, (db) => inspectBudgetForm(db, input));
  }

  importBudget(request: BudgetRequest): Promise<BudgetWriteResult> {
    return this.enqueue(async () => {
      const input = await this.budgetInput(request);
      const mutation = await this.mutateDatabase(
  ...
  importMeals(request: MealImportRequest): Promise<MealWriteResult> {
    return this.enqueue(async () => {
      const sourceChecksum = await sha256Text(request.sourceText);
      const mutation = await this.mutateDatabase(
        request.databasePath,
        'meals',
        (db) => writeMealInspection(db, {
            ...request,
            sourceChecksum,
            pluginVersion: this.pluginVersion,
          }),
        (result) => result.lifecycleState === 'ephemeral' ? 'ephemeral' : 'durable',
      );
      return {
        ...mutation.value,
        backupPath: mutation.backupPath,
  ...
  inspectMeals(request: MealInspectionRequest): Promise<MealInspection> {
  ...
  async inspectDaily(request: DailyRequest): Promise<DailyInspection> {
    const input = await this.dailyInput(request);
    return this.inspectDatabase(request.databasePath, (db) => inspectDailyNote(db, input));
  }

  importHistoricalDaily(request: DailyRequest): Promise<DailyWriteResult> {
    return this.enqueue(async () => {
      const input = await this.dailyInput(request);
      const mutation = await this.mutateDatabase(
        request.databasePath,
  ...
  async previewPlanning(request: PlanningRequest): Promise<PlanningSyncResult> {
  ...
  syncPlanning(request: PlanningRequest): Promise<PlanningSyncResult & BackupMutationMetadata> {
    return this.enqueue(async () => {
      const notes = await this.planningInputs(request.notes);
      const mutation = await this.mutateDatabase(
        request.databasePath,
  ...
  async inspectWeekly(request: WeeklyRequest): Promise<WeeklyImportResult> {
    const input = await this.weeklyInput(request);
    return this.inspectDatabase(request.databasePath, (db) => inspectWeeklyPlan(db, input));
  }

  importWeekly(request: WeeklyRequest): Promise<WeeklyWriteResult> {
    return this.enqueue(async () => {
      const input = await this.weeklyInput(request);
      const mutation = await this.mutateDatabase(
        request.databasePath,
  ...
  private async dailyInput(request: DailyRequest): Promise<DailyNoteInput> {
    return {
      ...request,
      sourceChecksum: await sha256Text(request.sourceText),
      pluginVersion: this.pluginVersion,
  ...
  private async weeklyInput(request: WeeklyRequest): Promise<WeeklyNoteInput> {
    return { ...request, sourceChecksum: await sha256Text(request.sourceText) };
  }

  private async budgetInput(request: BudgetRequest): Promise<BudgetFormInput> {
    return { ...request, sourceChecksum: await sha256Text(request.sourceText) };
  }

  private async planningInputs(notes: PlanningNoteRequest[]): Promise<PlanningNote[]> {
  ...
  private async mutateDatabase<T>(
    ...
    backupLabel: string,
    operation: (db: Database) => T,
    durability: DatabaseMutationDurability | ((value: T) => DatabaseMutationDurability) = 'durable',
    ...
    try {
      const value = runDatabaseTransaction(db, operation, enforceForeignKeysDuringMutation);
      ...
      const resolvedDurability = typeof durability === 'function' ? durability(value) : durability;
      const backupPath = shouldCreateDatabaseBackup(resolvedDurability)
  ...
  private async verifyWrittenDatabase(file: TFile, expectedBytes: Uint8Array): Promise<void> {
    ...
    try {
      ...
      verifyDatabaseIntegrity(verification);
    } finally {
      verification.close();
    }
  }
...
/** @deprecated Import LoggerService from logger/service.ts. */
export { LoggerService as NativeLoggerWriteService };
