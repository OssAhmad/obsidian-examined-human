import { findEntriesMarker, findFormSection } from '../forms/form-document.ts';
...
function sectionBody(content: string, name: string): string | null {
  return findFormSection(content, name)?.content ?? null;
}
...
function parseMealItems(db: Database, segment: string, type: MealType, errors: string[]): ParsedMealItem[] {
  const marker = findEntriesMarker(segment);
  if (!marker) {
  ...
  for (const originalLine of segment.slice(marker.end).split(/\r?\n/)) {
