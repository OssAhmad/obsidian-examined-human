...
import { findEntriesMarker, findFormSection, lineEndingFor, requireEhForm } from '../forms/form-document.ts';
import { labeledValue as findLabeledValue } from '../forms/fields.ts';
import {
  addIsoDays,
  assertSchemaV1,
  lastInsertId,
...
/** @deprecated Use WeeklyNoteInput. */
export type NativeWeeklyNoteInput = WeeklyNoteInput;
...
function weeklyFormBody(text: string): { body: string; weekStart: string } {
  const body = requireEhForm(
    text,
    'weekly',
    'Weekly note has no #### EH Weekly Form heading.',
    'EH Weekly Form has no matching #### END marker.',
  ).body;
...
function labeledValue(text: string, label: string): string | null {
  return findLabeledValue(text, label, true);
}
...
export function parseWeeklyPlan(db: Database, input: WeeklyNoteInput): ParsedWeeklyPlan {
  assertSchemaV1(db);
...
export function inspectWeeklyPlan(db: Database, input: WeeklyNoteInput): WeeklyImportResult {
...
export function writeWeeklyPlan(db: Database, input: WeeklyNoteInput): WeeklyImportResult {
...
function sessionEntryLocation(text: string): { position: number; prefix: string; occupied: boolean } {
  const form = requireEhForm(text, 'daily', 'no EH Daily Form heading', 'missing #### END');
  const formText = text.slice(form.headingStart, form.endStart);
  const section = findFormSection(formText, 'Sessions');
  if (!section) throw new Error('no Sessions section');
  const sectionText = formText.slice(section.contentStart, section.contentEnd);
  const entries = findEntriesMarker(sectionText);
  if (!entries) throw new Error('Sessions section has no ENTRIES marker');
  const markerEnd = form.headingStart + section.contentStart + entries.end;
  const newline = /^(\r?\n)/.exec(text.slice(markerEnd))?.[0] ?? '';
  const position = markerEnd + newline.length;
  const prefix = newline ? '' : lineEndingFor(text);
  const sectionEnd = form.headingStart + section.contentEnd;
