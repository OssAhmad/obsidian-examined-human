...
test('one-time v5 upgrade preserves legacy meal rows and resets official schema metadata', () => {
  ...
  try {
    ...
    assert.deepEqual(preview, { currentSchemaVersion: 5, targetSchemaVersion: 1, migrationEntryCount: 5 });
    applyV5ToOfficialSchemaV1(db, upgradeSql);
    assertMealImportSchema(db);
    assert.equal(db.exec('PRAGMA user_version')[0].values[0][0], 1);
    assert.deepEqual(db.exec('SELECT version, name FROM schema_migrations')[0].values, [[1, 'official schema v1: canonical food dictionary']]);
