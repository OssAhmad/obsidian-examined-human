...
test('creates an Admin Events section inside the bounded EH Daily Form when missing', () => {
  const source = `#### EH Daily Form\ndate: 2026-08-25\n\n##### Sessions\nENTRIES:\n\n#### END`;
  const preview = prepareAdminEventStage(input(source));
...
test('always terminates staged commands before the EH Daily Form end marker', () => {
  const preview = prepareAdminEventStage(input(`#### EH Daily Form
date: 2026-08-25
##### Admin Events
ENTRIES:
#### END`));
...
test('does not treat a private Admin Events heading outside EH Daily Form as writable', () => {
...
test('rejects a duplicate command and malformed EH Daily Form bounds', () => {
  assert.throws(
    () => prepareAdminEventStage(input(`#### EH Daily Form\ndate: 2026-08-25\n\n##### Admin Events\nENTRIES:\n${command}\n\n#### END`)),
    /already staged/,
  );
  assert.throws(() => prepareAdminEventStage(input('##### Admin Events\nENTRIES:')), /no #### EH Daily Form/);
  assert.throws(() => prepareAdminEventStage(input('#### EH Daily Form\ndate: 2026-08-25\n##### Admin Events')), /no matching #### END/);
});
